	...
	for {
		...
		case <-ticker.C:
			...
			for i := range jobs {
				...
				if err := db.UpdateJobStarted(jobs[i].ID); err != nil {
					continue
				}
...
func (jw *JobWorker) executeJob(job *db.Job) error {
	...
	if err != nil {
		_ = db.UpdateJobFailed(job.ID, "internal db error: "+err.Error())
		return err
	}

	var questions []string
	if err := json.Unmarshal(result.QuestionsExtracted, &questions); err != nil {
		_ = db.UpdateJobFailed(job.ID, "failed to parse extracted questions JSON")
		return err
	}

	// Safety Check: If there's nothing to process, skip OpenAI cleanly
	if len(questions) == 0 {
		_ = db.UpdateJobCompleted(job.ID)
		return nil
	}

	// Verify OpenAI configurations are present
	if jw.openaiKey == "" {
		_ = db.UpdateJobFailed(job.ID, "system error: OpenAI API key configuration missing")
	...
	if err != nil {
		...
		_ = db.UpdateJobFailed(job.ID, "OpenAI API timeout or rate-limit: "+err.Error())
	...
	if err != nil {
		...
		_ = db.UpdateJobFailed(job.ID, "OpenAI individual processing failed: "+err.Error())
	...
	if err != nil {
		_ = db.UpdateJobFailed(job.ID, "payload serialization crash")
		return err
	}
	...
	if err := db.DB.Model(&db.Result{}).Where("id = ?", job.ResultID).Update("ai_responses", responsesJSON).Error; err != nil {
		...
		_ = db.UpdateJobFailed(job.ID, "database write failure")
		return err
	}

	// ONLY MARK AS COMPLETED IF EVERYTHING ABOVE PASSED WITH ZERO ERRORS
	return db.UpdateJobCompleted(job.ID)
}
